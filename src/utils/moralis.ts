...
export const getBalancesMoralis = async (address: any, chain: any) => {
  ...
  const response = await Moralis.EvmApi?.token?.getWalletTokenBalances({ address, chain: selectChain })
  const responseNative = await Moralis.EvmApi?.balance?.getNativeBalance({ address, chain: selectChain })
  return [...response.result, { value: responseNative.result?.balance?.ether, token: selectChain.currency }]
