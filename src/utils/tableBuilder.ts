...
export const buildTable = ({
  ...
  <h1 style="font-size: 26px;line-height: 1.4;text-align: center;color: #fff">${translate(
    type,
  )}${name ? ` - ${name}` : ''}</h1>
  <h2 style="font-size: 18px;line-height: 1.4;text-align: center;color: #fff">${translate(
    'from',
  ...
  <table style="width: 100%;border-collapse: collapse; border-radius:10px;margin: 0 auto">
    <thead>
      ...
      ${headers
        .map((header: any) => {
          return `<th style="font-size: 18px;line-height: 1.4;text-align: center;width: 20%;background-color:#1E2A34;color: #fff">${translate(
            header,
          )}</th>`
        })
        .join('')}
  ...
  table += entries
    ...
    ${values
      .map((value: any) => {
        return `<td style="text-align: center;width: 20%;background-color: ${
          index % 2 === 0 ? '#F0F0F2' : '#B1C7D8'
        };color: #121E28; font-size: 15px">${movement[value]}</td>`
      })
      .join('')}
    </tr>`
    })
