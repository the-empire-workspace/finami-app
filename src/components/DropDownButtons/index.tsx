...
const DropDownButtons: FC<DropDownInfoProps> = ({ DropDownInfo }: any) => {
    ...
    if (selected)
      return (
        <View style={[styles.hidden, { backgroundColor: colors.background50 }]}>
          {!!DropDownInfo[0].label && <TouchableOpacity
          ...
          {!!DropDownInfo[1].label && <TouchableOpacity
            style={[styles.buttonItems, { backgroundColor: colors.background25 }]}
  ...
  return (
    <View
      ...
      {(!!DropDownInfo[0].label && !!DropDownInfo[1].label) && renderDropdown()}
      <View style={[styles.show]}>
        ...
        {(!!DropDownInfo[0].label && !!DropDownInfo[1].label) ? <TouchableOpacity onPress={() => setSelected(!selected)}>
          <DropDownArrow
            width={24}
            height={24}
