...
import { GET_INCOMING, GET_INCOMING_ASYNC, SET_INCOMING, SET_INCOMING_ASYNC } from './action-types'
...
export function* watchSetIncoming() {
  yield takeLatest(SET_INCOMING, setIncomingAsync)
}
export function* watchGetIncoming() {
  yield takeLatest(GET_INCOMING, getIncomingAsync)
}
