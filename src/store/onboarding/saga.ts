...
import { actionObject, createAccountQuery, createEntryQuery, createUserQuery } from 'utils'
...
function* completeOnboardingAsync({ payload }: any): any {
  try {
    ...
    const entry = yield call(createEntryQuery, { user: user?.id, account: account?.id, payment_type: 'general', amount: data?.available_balance, payment_concept: 'initial', entry_type: 'income', comment: '', emissor: '', email: '', phone: '', date: new Date()?.getTime() })
    yield put(signin())
