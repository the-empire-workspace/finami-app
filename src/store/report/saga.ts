...
import { selectAccount, selectCurrency, selectGoals, selectIncoming, selectOutcoming } from '../selector'
import { GENERATE_REPORT, GENERATE_REPORT_ASYNC } from './action-types'
import { getExpensesReportQuery, getGoalsReportQuery, getIncomesReportQuery, getLastMovementsQuery, getReportAccountQuery, getReportCurrenciesQuery, getReportFromEntryCategoryQuery, getReportFromEntryQuery } from 'utils/database/reportEntryQueries'
...
function* generateReportAsync({ payload }: any): any {
  ...
  try {
    ...
    switch (payload.type) {
      case 'last_movements':
        entries = yield call(getLastMovementsQuery, payload.from_date, payload.to_date)
        break
      case 'currency':
        entries = yield call(getReportCurrenciesQuery, payload.from_date, payload.to_date, payload?.id)
        break;
      case 'account':
        const { account = {} } = yield select(selectAccount)
        main = account
        entries = yield call(getReportAccountQuery, payload.from_date, payload.to_date, payload?.id)
        break;
      case 'incomes':
        entries = yield call(getIncomesReportQuery, payload.from_date, payload.to_date)
        break;
      case 'outcomes':
        entries = yield call(getExpensesReportQuery, payload.from_date, payload.to_date)
        break;
      case 'debt':
        const { item: debt } = yield select(selectOutcoming)
        main = debt
        entries = yield call(getReportFromEntryQuery, payload.from_date, payload.to_date, payload?.id)
        break;
      case 'debt_category':
        const { item: debt_category } = yield select(selectOutcoming)
        main = debt_category
        entries = yield call(getReportFromEntryCategoryQuery, payload.from_date, payload.to_date, payload?.id)
        break;
      case 'basic_expenses':
      ...
      case 'fixed_incomes_category':
        ...
        break;
      case 'compromise':
        entries = yield call(getGoalsReportQuery, payload.from_date, payload.to_date, payload?.type)
        break;
      case 'desire':
        entries = yield call(getGoalsReportQuery, payload.from_date, payload.to_date, payload?.type)
        break;
    ...
    entries.map((entry: any) => {
      ...
      entry.amount = `${entry.currency_symbol}${entry.amount?.toLocaleString('en-US', {
        maximumFractionDigits: entry?.decimal,
      })} ${entry.currency_name}`
      entry.entry_type = translate(entry.entry_type)
      return entry
    })
    table = buildTable({ type: payload?.type, from_date: payload?.from_date, to_date: payload?.to_date, entries, headers, values, name: main?.name || main?.payment_concept || main?.account_name || main?.currency_name })

    const pdf = yield call(PDFGenerator, table, 'report', 'Documents')
    yield call(RNFS.moveFile, pdf.filePath, `${RNFS.DownloadDirectoryPath}/report_${payload.type}.pdf`)
    FileViewer.open(`${RNFS.DownloadDirectoryPath}/report_${payload.type}.pdf`, { showOpenWithDialog: true })
  } catch (error) {
    console.log(error)
  }
}
