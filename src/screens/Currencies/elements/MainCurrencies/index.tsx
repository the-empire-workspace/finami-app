      ...
      <ScrollView contentContainerStyle={styles.scrollContainer}>
        {currencies?.map((item: any, index: any) => (
          <TouchableOpacity style={[styles.currencyItem]} key={index}>
            <Text style={[styles.strongBody, { color: (item?.total_amount === 0) ? colors.typography : colors.progress.ingress }]}>{item?.total_amount.toLocaleString('en-US', {
              minimumFractionDigits: (item?.total_amount > 0) ? item?.decimal : 0,
            })}</Text>
            <Text style={[styles.strongBody, { color: colors.typography }]}>{item?.name} {`(${item?.symbol})`}</Text>
