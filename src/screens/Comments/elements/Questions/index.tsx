...
const Questions: FC = () => {
  ...
  const setFile = async (file: any) => {
    ...
    setResponses({
      ...responses,
      [file]: {fileName, base64, height, width, type},
    })
  }
  ...
  return (
    <View style={[styles.root, {backgroundColor: colors.background100}]}>
        ...
        {question?.map((item, index) => (
          <View style={styles.questionContainer} key={index}>
            ...
            <View style={styles.buttonContainer}>
              {item?.type === 'number' ? (
                numberArray?.map((itemNumber: any, index2: any) => {
                  return (
                    <TouchableOpacity
                      key={index2}
                      style={[
                        ...
                        {
                          backgroundColor:
                            responses[item?.value] === index2 + 1
                              ? colors.typography
                              : colors.background100,
                        },
                      ]}
                      onPress={() =>
                        setResponses({...responses, [item?.value]: index2 + 1})
                      }>
                      <Text
                        style={[
                          styles.body,
                ...
                <TextInput
                  ...
                  placeholder={translate('suggestions')}
                  onChange={(val: any) =>
                    setResponses({
                      ...responses,
                      [item?.value]: val.nativeEvent.text,
                    })
                  }
                  editable
                  maxLength={200}
                ...
                <TouchableOpacity
                  style={[
                    styles.fileSelection,
                    {borderColor: colors.typography},
                  ]}
                  onPress={() => setFile(item?.value)}>
                  <View
                    style={[
      ...
      <View style={[styles.scrollContainer]}>
        <Button
          disabled={Object.keys(responses).reduce(
            (prev: any, curr: any) => prev || !responses[curr],
            false,
          )}
          text={translate('next')}
          onPress={() => {
