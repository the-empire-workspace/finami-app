...
const DetailPendingIncome: FC = () => {
  ...
  useEffect(() => {
    if (params?.id)
      if (params?.type === 'income') dispatch(getReceivableAccount(params?.id))
    ...
    <View style={[styles.root, { backgroundColor: colors.background100 }]}>
      <BackHandler
        navigation={
          item?.category_id
            ? () => {
              navigation.navigate('detailPendingIncome', {
                id: item?.category_id,
                type: 'category',
              })
            }
      ...
      <View style={[styles.mainInfo, { backgroundColor: colors.background50 }]}>
        <View style={[styles.progressContainer]}>
          ...
          <View
            style={[
              ...
              { borderColor: colors.progress.ingress },
            ]}>
            <View
              style={[
                styles.progress,
        ...
        {item?.limit_date && !shortInfo && (
          <View style={styles.accountData}>
            ...
            <Text style={[styles.smallBody, { color: colors.typography }]}>
              {new Date(item?.limit_date).toLocaleDateString((language === 'es') ? 'es-VE' : 'en-US')}
            </Text>
          </View>
        )}
        ...
        <View style={styles.actions}>
          <View style={[styles.actionContainer]}>
            <TouchableOpacity
              style={[styles.action]}
              onPress={() =>
                navigation.navigate('editPendingIncome', {
                  id: item?.id,
                  type: params?.type,
                })
              }>
...
export default DetailPendingIncome
