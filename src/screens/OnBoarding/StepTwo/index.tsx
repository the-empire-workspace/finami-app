...
const StepTwo: FC = () => {
  ...
  const [data, setData] = useState({ username: username || '', image: image || '' })
  ...
  const changeUsername = (data: any) => {
    if (data?.value?.username?.validation) setData((oldData) => ({ ...oldData, username: data.value?.username?.value }))
  }
    ...
    <ScrollView style={[{ backgroundColor: colors.background50 }]} contentContainerStyle={styles.scrollRoot}>
      <View style={styles.root}>
        <View style={styles.container}>
          ...
          <Text style={[styles.h2, styles.title, { color: colors.typography }]}>{translate('know_who_is_start')}</Text>
          <View style={[styles.container, styles.centerContainer]}>
            <Avatar defaultAvatar={(data?.image) ? { uri: data?.image } : null} actionAvatar={(data: any) => { setData((oldData) => ({ ...oldData, image: data })) }} />
          </View>
        ...
        <View style={[styles.container, styles.formContainer]}>
          <Button disabled={false} text={translate('next')} onPress={submitStep} />
        </View>
      </View>
    </ScrollView>
  )
