...
const EditEntry: FC = () => {
  ...
  const eForm = useMemo(() => egressForm(translate, values, colors), [item, accounts.length])
  const rForm = useMemo(() => receiverForm(translate, values, colors), [item, accounts.length])
  ...
  useEffect(() => {
    ...
    if (Object.keys(newValues).length > 0) setValues(newValues)
  ...
  const itemValues = useMemo(() => {
    ...
    return {
      title:
        ...
        translate('detail'),
      subtitle:
        subtitleSelection[item?.type || item?.entry_type] ||
        subtitleSelection[item?.entry_type] ||
        translate('detail'),
      color:
