...
const EditAccount: FC = () => {
  ...
  const form = useMemo(() => {
    ...
    return (
      formsTypes[values?.account_type?.value === 'wallet' ? 'connect' : values?.account_type?.value] || cryptoForm(translate, values, colors)
    )
      ...
      <View style={[styles.scrollContainer]}>
        <Button
          ...
          disabled={Object.keys(values)?.reduce((prev: any, next: any) => {
            return prev || (values?.[next]?.validation !== undefined) ? !values[next]?.validation : false
          }, false)}
        />
      </View>
    </View>
