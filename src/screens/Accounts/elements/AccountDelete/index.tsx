...
const AccountDelete: FC = () => {
  ...
  return (
    <View style={[styles.root, { backgroundColor: colors.background100 }]}>
      <ScrollView style={[styles.modalContainer]} contentContainerStyle={[styles.modalLayout]}>
        <WarningCircle width={64} height={64} />
        <Text style={[styles.h1, { color: colors.typography }]}>{translate('confirm_delete_information')}</Text>
        <Text style={[styles.h3, { color: colors.typography }]}>{translate('once_eliminated')}</Text>
      </ScrollView>
